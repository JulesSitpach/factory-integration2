...
export default function SupplierManagement() {
  ...
  }, [filters, pagination.limit, pagination.offset, t]);
  ...
  const handleEditSupplier = async (data: SupplierFormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || t('editError'));
      }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <h2 className="text-lg font-semibold">{t('supplierDirectory')}</h2>
          <button
            type="button"
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="text-center py-10 bg-gray-50 rounded-lg">
              ...
              <p className="mt-2 text-gray-700">{t('noSuppliers')}</p>
              <button
                type="button"
                onClick={openAddModal}
                      ...
                      <div className="flex items-center">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {t('added')}: {formatDate(supplier.created_at)}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-900">
                          {supplier.verified ? t('verified') : t('notVerified')}
                        </span>
                      </div>
                      {supplier.rating !== null && (
                        <div className="text-sm text-gray-500 mt-1">
                          {t('rating')}: {supplier.rating}/5
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4">
